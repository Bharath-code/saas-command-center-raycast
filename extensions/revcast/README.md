...
If no Stripe project exists, onboarding offers `View Demo Metrics`. That creates a local demo project and serves data from `src/lib/demo-data.ts` instead of calling Stripe.
...
- `src/failed.tsx`
- `src/projects.tsx`
- `src/lib/stripe-metrics-service.ts`
- `src/lib/storage.ts`
...
The website expects the variables in `website/.env.example`.
